...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const dprRound = (v: number) => Math.round(v * dpr) / dpr;
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      let scale = dprRound(rawScale);
      if (scale > 0.999 && scale < 1.001) scale = 1; // snap near 1
      ...
      if (isPinned) {
        translateY = dprRound(scrollTop - cardTop + stackPositionPx + itemStackDistance * i);
      } else if (scrollTop > pinEnd) {
        translateY = dprRound(pinEnd - cardTop + stackPositionPx + itemStackDistance * i);
      }

      const newTransform = {
        translateY,
        scale,
        rotation: dprRound(rotation),
        blur: dprRound(blur)
      };
